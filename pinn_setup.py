...
class PINN(nn.Module):
    ...
    def forward(self, X: torch.Tensor, net: nn.Sequential) -> torch.Tensor:
        """Apply input normalization and forward pass through given
        network."""
        X_d = torch.maximum(
            self.ub - self.lb, torch.tensor(1e-6, device=self.device)
        )
    ...
    def net_plasma(self, x: torch.Tensor, y: torch.Tensor, t: torch.Tensor):
        ...
        v1_t = torch.autograd.grad(
            v1, t, grad_outputs=torch.ones_like(v1), create_graph=True
        )[0]
        v1_x = torch.autograd.grad(
            v1, x, grad_outputs=torch.ones_like(v1), create_graph=True
        )[0]
        v1_y = torch.autograd.grad(
            v1, y, grad_outputs=torch.ones_like(v1), create_graph=True
        )[0]
        v2_x = torch.autograd.grad(
            v2, x, grad_outputs=torch.ones_like(v2), create_graph=True
        )[0]
        v2_y = torch.autograd.grad(
            v2, y, grad_outputs=torch.ones_like(v2), create_graph=True
        )[0]
        v5_t = torch.autograd.grad(
            v5, t, grad_outputs=torch.ones_like(v5), create_graph=True
        )[0]
        v5_x = torch.autograd.grad(
            v5, x, grad_outputs=torch.ones_like(v5), create_graph=True
        )[0]
        v5_y = torch.autograd.grad(
            v5, y, grad_outputs=torch.ones_like(v5), create_graph=True
        )[0]
    ...
    def compute_residuals(self, v1, v5, v1_t, v1_x, v1_y, v2_x, v2_y,
                          v5_t, v5_x, v5_y, B, pe_y, jp, pe, D_lnn, D_lnTe,
                          S_n, S_Ee):
    ...
    def train_step(self, loss_fn, optimizer_v1, optimizer_v5,
                   optimizer_f=None):
        ...
        v1_pred, v5_pred, _, _, _, f_v1, f_v5 = self.net_plasma(
            x_b, y_b, t_b
        )
    ...
    def predict(self, x_star: np.ndarray, y_star: np.ndarray,
        ...
        v1, v5, v2, v3, v4, _, _ = self.net_plasma(
        ...
        if self.use_pde:
            result.update({
                'v2': v2.cpu().numpy(), 'v3': v3.cpu().numpy(),
                'v4': v4.cpu().numpy()
            })
        return result
    ...
    def load(self, load_path: str):
        ...
        self.lb = torch.tensor(
            metadata['lb'], dtype=torch.float32
        ).to(self.device)
        self.ub = torch.tensor(
            metadata['ub'], dtype=torch.float32
        ).to(self.device)
